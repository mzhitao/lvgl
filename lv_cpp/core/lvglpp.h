...
template <typename ManagedType, auto Functor> using LvPointer = std::unique_ptr<ManagedType, CustomDeleter<Functor>>;

/* Used for create a smart_pointer with default deleter for cpp object */
template <typename ManagedType> using LvPointerUnique = std::unique_ptr<ManagedType>;

/* Declaring Events using smart pointers */
template <typename Class> using LvEventPointer = LvPointerUnique<LvEventCb<Class>>;

/* Variadic class contructor */
template <typename Class, typename... ArgsT> std::unique_ptr<Class> Make(ArgsT... args)
{
