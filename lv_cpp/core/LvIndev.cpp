...
LvInput::LvInput() : LvInput(NULL)
{}

LvInput::LvInput(lv_indev_t * drv)
{

    lv_indev_get_next(&indev_drv); /*Basic initialization*/
    ...
    if(!drv) {
        ...
        memcpy(&indev_drv, drv, sizeof(lv_indev_t));
    }

    indev.reset(lv_indev_get_next(&indev_drv));
}
...
LvInput & LvInput::enableCursor()
{
    ...
    if(!Cursor.get()) Cursor = Make<LvImg>();
