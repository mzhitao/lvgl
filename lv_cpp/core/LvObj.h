...
class LvObj {
  ...
  public:
    ...
    lv_style_res_t getLocalStyleProp(lv_style_prop_t prop, lv_style_value_t * value,
                                     lv_style_selector_t selector) const noexcept;
    bool removeLocalStyleProp(lv_style_prop_t prop, lv_style_selector_t selector);
    LvObj & styleCreateTransition(lv_part_t part, lv_state_t prev_state, lv_state_t new_state,
                                  const _lv_obj_style_transition_dsc_t * tr_dsc);
    lv_style_state_cmp_t styleStateCompare(lv_state_t state1, lv_state_t state2);
    ...
    LvObj & initDrawImgDsc(lv_part_t part, lv_draw_image_dsc_t * draw_dsc);
    ...
    LvObj & setGridCell(lv_grid_align_t x_align, uint8_t col_pos, uint8_t col_span, lv_grid_align_t y_align,
