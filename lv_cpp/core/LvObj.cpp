...
{
    return _LV_DPX_CALC(lv_disp_get_dpi(lv_obj_get_disp(raw())), n);
}
LvObj & LvObj::del()
{
    lv_obj_del(cObj.get());
...
lv_style_res_t LvObj::getLocalStyleProp(lv_style_prop_t prop, lv_style_value_t * value,
                                        lv_style_selector_t selector) const noexcept
{
...
LvObj & LvObj::styleCreateTransition(lv_part_t part, lv_state_t prev_state, lv_state_t new_state,
                                     const _lv_obj_style_transition_dsc_t * tr_dsc)
{
    lv_obj_style_create_transition(cObj.get(), part, prev_state, new_state, tr_dsc);
    return *this;
}
lv_style_state_cmp_t LvObj::styleStateCompare(lv_state_t state1, lv_state_t state2)
{
    return lv_obj_style_state_compare(cObj.get(), state1, state2);
}
...
LvObj & LvObj::initDrawImgDsc(lv_part_t part, lv_draw_image_dsc_t * draw_dsc)
{
...
LvObj & LvObj::setGridCell(lv_grid_align_t x_align, uint8_t col_pos, uint8_t col_span, lv_grid_align_t y_align,
...
void LvObj::EventDispatcher(lv_event_t * e)
{
    ...
    if(_Obj) {
        ...
        for(eventIter = _Obj->eventRegister.begin(); eventIter != _Obj->eventRegister.end(); eventIter++) {
            if(eventIter->evCode == e->code) {
                /* Call the event */
                if(eventIter->evCallback) (*(eventIter->evCallback))(e);
            }
        }
    }
}
